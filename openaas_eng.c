...
int main(int argc, char *argv[]) {
	...
	retval = call_CreateSubModel(serverIP, SM_AAS_ID_STRING, SM_AAS_ID_TYPE,
			SM_PARENT_ID_STRING, SM_PARENT_ID_TYPE, SM_MODEL_ID_STRING,
			SM_MODEL_ID_TYPE, SM_NAME, SM_REVISION, SM_VERSION);
	if (retval != UA_STATUSCODE_GOOD) {
	...
	retval = call_CreatePVSL(serverIP, PVSL_AAS_ID_STRING, PVSL_AAS_ID_TYPE,
			...
			PVSL_CARRIER_ID_STRING, PVSL_CARRIER_ID_TYPE, PVSL_EXPRESSIONLOGIC,
			PVSL_EXPRESSIONSEMANTIC, PVSL_PROPERTY_ID_STRING,
			PVSL_PROPERTY_ID_TYPE, PVSL_VIEW, PVSL_VISIBILITY);
	if (retval != UA_STATUSCODE_GOOD) {
	...
	retval = call_CreatePVS(serverIP, PVS_AAS_ID_STRING, PVS_AAS_ID_TYPE,
			PVS_LIST_ID_STRING, PVS_LIST_ID_TYPE, PVS_NAME, PVS_VALUE,
			PVS_VALUETYPE, PVS_MASK, PVS_CARRIER_ID_STRING, PVS_CARRIER_ID_TYPE,
			PVS_EXPRESSIONLOGIC, PVS_EXPRESSIONSEMANTIC, PVS_PROPERTY_ID_STRING,
			PVS_PROPERTY_ID_TYPE, PVS_VIEW, PVS_VISIBILITY);
	if (retval != UA_STATUSCODE_GOOD) {
	...
	retval = call_GetPVS(serverIP, PVS_GET_AAS_ID_STRING, PVS_GET_AAS_ID_TYPE,
			PVS_GET_ID_STRING, PVS_GET_ID_TYPE, &PVS_GET_PVSName,
			&PVS_GET_CarrierIdSpec, &PVS_GET_CarrierIdType,
			&PVS_GET_ExpressionLogic, &PVS_GET_ExpressionSemantic,
			&PVS_GET_propertyIdSpec, &PVS_GET_propertyIdType, &PVS_GET_view,
			&PVS_GET_visibility, &PVS_GET_Value, &PVS_GET_ValueType);
	if (retval != UA_STATUSCODE_GOOD) {
	...
	printf("PVS received \n");
	printf("pvs name: %s \n", PVS_GET_PVSName);
	printf("pvs carrierId: %s, %i \n", PVS_GET_CarrierIdSpec,
			PVS_GET_CarrierIdType);
	printf("pvs Expression logic: %i \n", PVS_GET_ExpressionLogic);
	printf("pvs Expression semantic: %i \n", PVS_GET_ExpressionSemantic);
	printf("pvs propertyId: %s, %i \n", PVS_GET_propertyIdSpec,
			PVS_GET_propertyIdType);
	printf("pvs view: %i \n", PVS_GET_view);
	printf("pvs visibility: %i \n", PVS_GET_visibility);
	printf("pvs value: %s, %i \n", PVS_GET_Value, PVS_GET_ValueType);

	retval = call_SetPVS(serverIP, PVS_SET_AAS_ID_STRING, PVS_SET_AAS_ID_TYPE,
			PVS_SET_ID_STRING, PVS_SET_ID_TYPE, PVS_SET_MASK, PVS_SET_NAME,
			PVS_SET_CARRIER_ID_STRING, PVS_SET_CARRIER_ID_TYPE,
			PVS_SET_EXPRESSIONLOGIC, PVS_SET_EXPRESSIONSEMANTIC,
			PVS_SET_PROPERTY_ID_STRING, PVS_SET_PROPERTY_ID_TYPE, PVS_SET_VIEW,
			PVS_SET_VISIBILITY, PVS_SET_VALUE, PVS_SET_VALUETYPE);
	if (retval != UA_STATUSCODE_GOOD) {
	...
	retval = call_CreateLCE(serverIP, LCE_AAS_ID_STRING, LCE_AAS_ID_TYPE,
			LCE_CREATINGINSTANCE_ID_STRING, LCE_CREATINGINSTANCE_ID_TYPE,
			LCE_WRITINGINSTANCE_ID_STRING, LCE_WRITINGINSTANCE_ID_TYPE,
			LCE_EVENTCLASS, LCE_SUBJECT, LCE_TIMESTAMP, LCE_VALUE,
			LCE_VALUETYPE);
	if (retval != UA_STATUSCODE_GOOD) {
	...
	retval = call_DeleteLCE(serverIP, LCE_DEL_AAS_ID_STRING,
			LCE_DEL_AAS_ID_TYPE, LCE_ID);
	if (retval != UA_STATUSCODE_GOOD) {
	...
	retval = call_GetLCESimple(serverIP, LCE_GET_AAS_ID_STRING,
			LCE_GET_AAS_ID_TYPE, LCE_GET_ID, &LCE_GET_CreatingInstanceIdSpec,
			&LCE_GET_CreatingInstanceIdType, &LCE_GET_WritingInstanceIdSpec,
			&LCE_GET_WritingInstanceIdType, &LCE_GET_EventClass,
			&LCE_GET_Subject, &LCE_GET_TimeStamp, &LCE_GET_Value,
			&LCE_GET_ValueType);
	if (retval != UA_STATUSCODE_GOOD) {
	...
	printf("#%i ----\n", LCE_GET_ID);
	printf("lce creating instance: %s, %i \n", LCE_GET_CreatingInstanceIdSpec,
			LCE_GET_CreatingInstanceIdType);
	printf("lce writing instance: %s, %i \n", LCE_GET_WritingInstanceIdSpec,
			LCE_GET_WritingInstanceIdType);
	...
	retval = call_GetLCE(serverIP, LCE_GET_AAS_ID_STRING, LCE_GET_AAS_ID_TYPE,
			LCE_GET_ID, &LCE_GET_lceData);
	if (retval != UA_STATUSCODE_GOOD) {
	...
	printf("#%i ----\n", LCE_GET_ID);
	printf("lce creating instance: %s, %i \n",
			LCE_GET_lceData.creatingInstanceSpec,
			LCE_GET_lceData.creatingInstanceType);
	printf("lce writing instance: %s, %i \n",
			LCE_GET_lceData.writingInstanceSpec,
			LCE_GET_lceData.writingInstanceType);
	 ...
	 */
	lifeCycleEntryType LCE_SET_lceData;
	LCE_SET_lceData.creatingInstanceSpec = malloc(
			sizeof(LCE_SET_CREATINGINSTANCE_ID_STRING));
	...
	LCE_SET_lceData.writingInstanceSpec = malloc(
			sizeof(LCE_SET_WRITINGINSTANCE_ID_STRING));

	strcpy(LCE_SET_lceData.creatingInstanceSpec,
			LCE_SET_CREATINGINSTANCE_ID_STRING);
	...
	strcpy(LCE_SET_lceData.writingInstanceSpec,
			LCE_SET_WRITINGINSTANCE_ID_STRING);
	LCE_SET_lceData.writingInstanceType = LCE_SET_WRITINGINSTANCE_ID_TYPE;

	retval = call_SetLCE(serverIP, LCE_SET_AAS_ID_STRING, LCE_SET_AAS_ID_TYPE,
			LCE_SET_ID, LCE_SET_lceData);
	if (retval != UA_STATUSCODE_GOOD) {
	...
	int lcecount = call_GetLastLCEs(serverIP, LCE_GET_LAST_AAS_ID_STRING,
			...
			&LCE_GET_LAST_lastLCEsCount, &LCE_GET_LAST_lceData);
	printf("------------------------------\n");
	printf("lifecycle\n");
	...
	for (int i = 0; i < LCE_GET_LAST_lastLCEsCount; i++) {
		...
		printf("lce writing instance: %s, %i \n",
				LCE_GET_LAST_lceData[i].writingInstanceSpec,
				LCE_GET_LAST_lceData[i].writingInstanceType);
		printf("lce creating instance: %s, %i \n",
				LCE_GET_LAST_lceData[i].creatingInstanceSpec,
				LCE_GET_LAST_lceData[i].creatingInstanceType);
		printf("lce subject: %s \n", LCE_GET_LAST_lceData[i].subject);
		printf("lce eventClass: %s \n", LCE_GET_LAST_lceData[i].eventClass);
		printf("lce dataType: %i \n", LCE_GET_LAST_lceData[i].dataType);
		printf("lce data: %s, %i \n", LCE_GET_LAST_lceData[i].data,
				LCE_GET_LAST_lceData[i].dataType);
		printf("lce timestamp" "%" PRId64 "\n",
